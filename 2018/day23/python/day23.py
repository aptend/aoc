...
def bots_from_string(s):
    ...
    for line in lines:
        ...
        bots.append(bot(*digits))
    return bots

def bots_from_file():
    with open('../inputs.txt') as f:
        s = f.read()
    return bots_from_string(s)


def dist(p, q):
    return abs(p.x - q.x) + abs(p.y - q.y) + abs(p.z - q.z)

def largest_radius_signals(bots):
    p = max(bots, key=lambda p: p.r)
    return sum(1 for q in bots if dist(p, q) <= p.r)
...
pos=<1,3,1>, r=1
"""
# print(largest_radius_signals(bots_from_string(example)))
print(largest_radius_signals(bots_from_file()))
...
def find(done, bots, xs, ys, zs, dist, ox, oy, oz, forced_count):
    ...
    while len(at_target) > 0:
        ...
        if dist == 1:
            ...
            a, b = find(done, bots, xs, ys, zs, dist //
                        2, ox, oy, oz, forced_count)
            if a is None:
